...
#Création du dossier de partage utilisateur
try
{
New-Item -Itemtype Directory -Path "E:\Partages_personnels_utilisateurs\$login" -ErrorAction stop
Write-Host "Création du dossier partagé réussi" -ForegroundColor Green
}
catch
{
Write-Host " Une erreur est survenue lors de la création du dossier" -ForegroundColor Red
exit 1
}

try
{
New-SmbShare -Name $nom_complet -Path "E:\Partages_personnels_utilisateurs\$login" -FullAccess "Administrateurs" -ReadAccess "Utilisateurs" -ErrorAction stop    #Création du partage administrateur
Write-Host "Partage du dossier réussi" -ForegroundColor Green
}
catch
{
Write-Host " Une erreur est survenue lors du partage du dossier" -ForegroundColor Red
exit 1
}

try
{
Get-NTFSAccess -Path "E:\Partages_personnels_utilisateurs\$login"  -ErrorAction stop #Permet de lister les autorisations NTFS d'un répertoire 
Write-Host "Liste des autorisations NTFS crée" -ForegroundColor Green
}
catch
{
...
try
{
Add-NTFSAccess -Path "E:\Partages_personnels_utilisateurs\$login" -Account "$login" -AccessRights Full -ErrorAction stop  #Permet de modifier les droits d'utilisateur

Write-Host "Modification des droits d'utilisateurs réussi" -ForegroundColor Green
}
catch
{
Write-Host "Une erreur est survenue" -ForegroundColor Red
